...
function fdiskAndMount(){
  ...
  ansible $GROUP -m shell -a "mkfs.xfs -f /dev/vdb" 
  ansible $GROUP -m shell -a "mkdir -p /data && mount /dev/vdb /data" 
  ansible $GROUP -m shell -a "echo \"/dev/vdb /data xfs defaults 0 0\" >> /etc/fstab" 
}
#修改时区,同步时间
function setUTC(){
